...
def serialize_post_optimized(post, need_comments=True):
    return {
        'title': post.title,
        'teaser_text': post.text[:200],
        'author': post.author.username,
...
def index(request):
    ...
    context = {
        'most_popular_posts': [
            serialize_post_optimized(post, False) for post in most_popular_posts
        ],
        'page_posts': [serialize_post_optimized(post) for post in most_fresh_posts],
        'popular_tags': [serialize_tag_with_count(tag) for tag in most_popular_tags],
    }
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].select_related('author').\
        prefetch_related('tags').fetch_with_comments_count()
    context = {
        'tag': tag.title,
        'popular_tags': [serialize_tag_with_count(tag) for tag in most_popular_tags],
